...
pub fn list_local(base_path: &str) {
    ...
    println!("{}local list sorted local len(): {}",term_cursor::Goto(0,18), sorted_string.len());
    // end region: sort
...
pub fn move_or_rename_local_files(){
    ...
    let list_for_download = std::fs::read_to_string(path_list_for_download).unwrap();

    // I will replace the path inside the list_local_file.csv
    let mut string_local_files = std::fs::read_to_string("temp_data/list_local_files.csv").unwrap();
...
pub fn add_just_downloaded_to_list_local() {    
    ...
    let string_just_downloaded = std::fs::read_to_string(path_list_just_downloaded).unwrap();
    if !string_just_downloaded.is_empty(){
        ...
        unwrap!(std::fs::write(path_list_just_downloaded,&string_sorted_just_downloaded));    

        let path_list_local_files = "temp_data/list_local_files.csv";
        let string_local_files = std::fs::read_to_string(path_list_local_files).unwrap();
        ...
        unwrap!(std::fs::write(path_list_local_files, &new_local_files));
